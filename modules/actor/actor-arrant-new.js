 ...
 *
 * ActorArrant contains all the preparation data and methods used for preparing an actor:
 ...
 *
 *
 * @see   ActorSheetArrant - Base sheet class
 ...
 * @see   ChatArrant - Sends test data to roll tests.
 */
export default class ActorArrantNew extends Actor {
    ...
    async _preCreate(data, options, user) {
        ...
        let migration = game.arrant.migration.migrateActorData(this)
        this.updateSource({effects : game.arrant.migration.removeLoreEffects(data)}, {recursive : false});
        ...
        if (!data.items?.length)
            ...
            createData.items = this.items.map(i => mergeObject(i.toObject(), game.arrant.migration.migrateItemData(i), {overwrite : true}))

        if (data.effects?.length)
            createData.effects = this.effects.map(i => mergeObject(i.toObject(), game.arrant.migration.migrateEffectData(i), {overwrite : true}))
            ...
            mergeObject(createData,
                {
                    ...
                    "prototypeToken.texture.src" : "systems/arrant/tokens/unknown.png"      // Set token image
                })
        ...
        if (!data.img || data.img == "icons/svg/mystery-man.svg") {
            createData.img = "systems/arrant/tokens/unknown.png"
            if (data.type == "vehicle")
                createData.img = "systems/arrant/tokens/vehicle.png"
        }
    ...
    async _preUpdate(updateData, options, user) {
        ...
        if (!options.skipGroupAdvantage && hasProperty(updateData, "system.status.advantage.value") && game.settings.get("arrant", "useGroupAdvantage"))
        {
        ...
        if (this.prototypeToken?.texture?.src == "systems/arrant/tokens/unknown.png" && updateData.img) {
    ...
    get actorEffects() {
        ...
        effects.forEach(e => {
            ...
            try {
                ...
                game.arrant.utility.log(`The effect ${e.name} threw an error when being prepared. ${error}`, e)
            }
        })

        if (this.flags.arrant?.conditionalEffects?.length)
        {
            this.flags.arrant?.conditionalEffects.map(e => new EffectArrant(e, {parent: this})).forEach(e => {
    ...
    prepareNonVehicle() {
        ...
        {
            if (game.settings.get("arrant", "capAdvantageIB")) {
        ...
        if (trait)
            size = WFRP_Utility.findKey(trait.specification.value, game.arrant.config.actorSizes);
        ...
        if (this.isMounted && !game.actors) {
            game.arrant.postReadyPrepare.push(this);
        }
    ...
    prepareCharacter() {
        ...
        if (currentCareer) {
            ...
            this.details.status.value = game.arrant.config.statusTiers[this.details.status.tier] + " " + this.details.status.standing
        }
        else
  ...
  /* --------------------------------------------------------------------------------------------------------- */
    ...
    async setupDialog({ dialogOptions, testData, cardOptions }, type) {
        ...
        dialogOptions.data.difficultyLabels = game.arrant.config.difficultyLabels;
    ...
    async  setupCharacteristic(characteristicId, options = {}) {
        ...
        let testData = {
            title,
            rollClass: game.arrant.rolls.CharacteristicTest,
            ...
            hitLocationTable : game.arrant.tables.getHitLocTable(game.user.targets.values().next().value?.actor.details.hitLocationTable.value || "hitloc"),
        ...
        let dialogOptions = {
            title: title,
            template: "/systems/arrant/templates/dialog/characteristic-dialog.hbs",
            ...
            callback: (html) => {
                ...
                testData.testDifficulty = game.arrant.config.difficultyModifiers[html.find('[name="testDifficulty"]').val()];
        ...
        let cardOptions = this._setupCardOptions("systems/arrant/templates/chat/roll/characteristic-card.hbs", title)
    ...
    async setupSkill(skill, options = {}) {
        ...
        let testData = {
            title,
            rollClass: game.arrant.rolls.SkillTest,
        ...
        let dialogOptions = {
            title: title,
            template: "/systems/arrant/templates/dialog/skill-dialog.hbs",
            // Prefilled dialog data
        ...
        // Call the universal cardOptions helper
        let cardOptions = this._setupCardOptions("systems/arrant/templates/chat/roll/skill-card.hbs", title)
    ...
    async setupWeapon(weapon, options = {}) {
        ...
        let testData = {
            title,
            rollClass: game.arrant.rolls.WeaponTest,
            hitLocation: "roll",
        ...
        let dialogOptions = {
            title: title,
            template: "/systems/arrant/templates/dialog/weapon-dialog.hbs",
            // Prefilled dialog data
            data: {
                hitLocation: testData.hitLocation,
                talents: this.getTalentTests(),
        ...
        // Call the universal cardOptions helper
        let cardOptions = this._setupCardOptions("systems/arrant/templates/chat/roll/weapon-card.hbs", title)
    ...
    async setupCast(spell, options = {}) {
        ...
        let testData = {
            title,
            rollClass: game.settings.get("arrant", "useWoMOvercast") ? game.arrant.rolls.WomCastTest : game.arrant.rolls.CastTest,
            item: spell.id,
        ...
        //@HOUSE
        testData.unofficialGrimoire = game.settings.get("arrant", "unofficialgrimoire");
        ...
        let dialogOptions = {
            title: title,
            template: "/systems/arrant/templates/dialog/spell-dialog.hbs",
            // Prefilled dialog data
            data: {
                hitLocation: testData.hitLocation,
        ...
        //@HOUSE
        if (game.settings.get("arrant", "mooMagicAdvantage")) {
            game.arrant.utility.logHomebrew("mooMagicAdvantage")
            dialogOptions.data.advantage = "N/A"
        }
        //@/HOUSE

        // Call the universal cardOptions helper
        let cardOptions = this._setupCardOptions("systems/arrant/templates/chat/roll/spell-card.hbs", title)
    ...
    async setupChannell(spell, options = {}) {
        ...
        if (spell.wind && spell.wind.value) {
            ...
            defaultSelection = channellSkills.indexOf(channellSkills.find(x => x.name.includes(game.arrant.config.magicWind[spellLore])));
        }
        ...
        let testData = {
            title,
            rollClass: game.arrant.rolls.ChannelTest,
            item: spell.id,
            malignantInfluence: false,
        ...
        let dialogOptions = {
            title: title,
            template: "/systems/arrant/templates/dialog/channel-dialog.hbs",
            // Prefilled dialog data
            data: {
                malignantInfluence: testData.malignantInfluence,
        ...
        // Call the universal cardOptions helper
        let cardOptions = this._setupCardOptions("systems/arrant/templates/chat/roll/channel-card.hbs", title)
    ...
    async setupPrayer(prayer, options = {}) {
        ...
        let testData = { // Store this data to be used in the test logic
            title,
            rollClass: game.arrant.rolls.PrayerTest,
            item: prayer.id,
            hitLocation: false,
            options: options,
        ...
        let dialogOptions = {
            title: title,
            template: "/systems/arrant/templates/dialog/prayer-dialog.hbs",
            // Prefilled dialog data
            data: {
                hitLocation: testData.hitLocation,
                talents: this.getTalentTests(),
        ...
        // Call the universal cardOptions helper
        let cardOptions = this._setupCardOptions("systems/arrant/templates/chat/roll/prayer-card.hbs", title)
    ...
    async setupTrait(trait, options = {}) {
        ...
        let title = options.title || game.arrant.config.characteristics[trait.rollable.rollCharacteristic] + ` ${game.i18n.localize("Test")} - ` + trait.name;
        title += options.appendTitle || "";
        ...
        let testData = {
            title,
            rollClass: game.arrant.rolls.TraitTest,
        ...
        let dialogOptions = {
            title: title,
            template: "/systems/arrant/templates/dialog/skill-dialog.hbs", // Reuse skill dialog
            // Prefilled dialog data
            data: {
                hitLocation: testData.hitLocation, // Empty string = "roll"
                talents: this.getTalentTests(),
    ...
    async setupExtendedTest(item, options = {}) {
        ...
        let characteristic = WFRP_Utility.findKey(item.test.value, game.arrant.config.characteristics)
        if (characteristic) {
    ...
    _setupCardOptions(template, title) {
        ...
        if (this.token) {
            ...
            if (this.token.hidden) {
                ...
                cardOptions.flags.img = "systems/arrant/tokens/unknown.png"
            }
        }
    ...
    async rollReloadTest(weapon) {
        let testId = weapon.getFlag("arrant", "reloading")
    ...
    _getTokenSize() {
        ...
        let tokenSize = game.arrant.config.tokenSizes[this.details.size.value];
        if (tokenSize < 1)
        {
     ...
     */
    async applyDamage(opposedTest, damageType = game.arrant.config.DAMAGE_TYPE.NORMAL) {
        ...
        let applyAP = (damageType == game.arrant.config.DAMAGE_TYPE.IGNORE_TB || damageType == game.arrant.config.DAMAGE_TYPE.NORMAL)
        let applyTB = (damageType == game.arrant.config.DAMAGE_TYPE.IGNORE_AP || damageType == game.arrant.config.DAMAGE_TYPE.NORMAL)
        ...
        let messageElements = []
        let extraMessages = [];
        // if (damageType !=  game.arrant.config.DAMAGE_TYPE.IGNORE_ALL)
        //   updateMsg += " ("
        ...
        if (applyAP) {
            ...
            //@HOUSE
            if (penetrating && game.settings.get("arrant", "mooPenetrating")) {
                game.arrant.utility.logHomebrew("mooPenetrating")
            ...
            let shieldAP = 0;
            if (game.settings.get("arrant", "uiaShields") && !opposedTest.defenderTest.context.unopposed) // UIA shields don't need to be used, just equipped
            {
            ...
            //@HOUSE
            if (game.settings.get("arrant", "mooShieldAP") && opposedTest.defenderTest.result.outcome == "failure") {
                game.arrant.utility.logHomebrew("mooShieldAP")
                shieldAP = 0;
            }
            //@/HOUSE
        ...
        if (newWounds <= 0) {
            ...
            let critAmnt = game.settings.get("arrant", "uiaCritsMod")
            if (game.settings.get("arrant", "uiaCrits") && critAmnt && (Math.abs(newWounds)) > 0) {
            ...
            //@HOUSE
            else if (game.settings.get("arrant", "mooCritModifiers")) {
                game.arrant.utility.logHomebrew("mooCritModifiers")
            ...
            //@/HOUSE
            else if (Math.abs(newWounds) < actor.characteristics.t.bonus && !game.settings.get("arrant", "uiaCrits"))
        ...
        if (hack)
            updateMsg += `<br>${game.i18n.localize("CHAT.DamageAP")} ${game.arrant.config.locations[opposedTest.result.hitloc.value]}`
     ...
     */
    async applyBasicDamage(damage, { damageType = game.arrant.config.DAMAGE_TYPE.NORMAL, minimumOne = true, loc = "body", suppressMsg = false } = {}) {
        ...
        if (loc == "roll")
        {
            loc = (await game.arrant.tables.rollTable("hitloc")).result
        }
    ...
    async _advanceSpeciesSkills() {
        ...
        try {
            let { skills } = game.arrant.utility.speciesSkillsTalents(this.details.species.value, this.details.species.subspecies)
            skillList = skills
            if (!skillList) {
    ...
    async _advanceSpeciesTalents() {
        ...
        let talentList
        try {
            let { talents } = game.arrant.utility.speciesSkillsTalents(this.details.species.value, this.details.species.subspecies)
            talentList = talents
            if (!talentList) {
            }
        }
        ...
        for (let talent of talentList) {
            if (!isNaN(talent)) // If is a number, roll on random talents
            {
                for (let i = 0; i < talent; i++) {
                    let result = await game.arrant.tables.rollTable("talents")
                ...
                endurance: {
                    ...
                    callback: () => {
                        ...
                        if (skill) {
                            ...
                            this.setupCharacteristic("t", { title: game.i18n.format("DIALOG.CorruptionTestTitle", { test: game.arrant.config.characteristics["t"] }), corruption: strength }).then(setupData => this.basicTest(setupData))
                        }
                    }
                },
                cool: {
                    ...
                    callback: () => {
                        ...
                        if (skill) {
                            ...
                            this.setupCharacteristic("wp", { title: game.i18n.format("DIALOG.CorruptionTestTitle", { test: game.arrant.config.characteristics["wp"] }), corruption: strength }).then(setupData => this.basicTest(setupData))
                        }
                    }
                }
    ...
    async getPrefillData(type, item, options = {}) {
        ...
        try {
            ...
            if (game.settings.get("arrant", "testDefaultDifficulty") && (game.combat != null))
                difficulty = game.combat.started ? "challenging" : "average";
            else if (game.settings.get("arrant", "testDefaultDifficulty"))
                difficulty = "average";
            ...
            if (this.type != "vehicle") {
                ...
                // @HOUSE
                if (type == "channelling" && game.settings.get("arrant", "mooMagicAdvantage"))
                    addAdvantage = true;

                if (type == 'cast' && game.settings.get("arrant", "mooMagicAdvantage"))
                    addAdvantage = false
                // @/HOUSE
                ...
                if (addAdvantage) {
                    ...
                    if (!game.settings.get("arrant", "mooAdvantage")) {
                        modifier += game.settings.get("arrant", "autoFillAdvantage") ? (this.status.advantage.value * game.settings.get("arrant", "advantageBonus") || 0) : 0
                        if (parseInt(this.status.advantage.value) && game.settings.get("arrant", "autoFillAdvantage"))
                            tooltip.push(`${game.i18n.localize("Advantage")} (+${(this.status.advantage.value * game.settings.get("arrant", "advantageBonus"))})`);
                    }
                    // @HOUSE - +1 Success Bonus SL per advantage
                    else if (game.settings.get("arrant", "mooAdvantage")) {
                        successBonus += game.settings.get("arrant", "autoFillAdvantage") ? (this.status.advantage.value * 1 || 0) : 0
                        if (parseInt(this.status.advantage.value) && game.settings.get("arrant", "autoFillAdvantage"))
            ...
            if (attacker && attacker.test.item.properties?.flaws.slow) {
                if (!game.settings.get("arrant", "mooQualities") || ((type == "skill" && item.name == game.i18n.localize("NAME.Dodge")) || (type == "characteristic" && options.dodge))) {
                    slBonus += 1
            ...
            if (options.modify) {
                ...
                if (options.modify.difficulty)
                    difficulty = game.arrant.utility.alterDifficulty(difficulty, options.modify.difficulty)
    ...
    rangePrefillModifiers(weapon, options, tooltip = []) {
        ...
        if (!game.settings.get("arrant", "rangeAutoCalculation") || !token || !game.user.targets.size == 1 || !weapon.range?.bands)
            return 0
    ...
    sizePrefillModifiers(item, type, options, tooltip) {
        ...
        try {
            ...
            if (attacker) {
                //Size Differences
                let sizeDiff = game.arrant.config.actorSizeNums[attacker.test.size] - this.sizeNum
                ...
                if (sizeDiff < 0 && (item.attackType == "melee" || target.sizeNum <= 3)) {
                    ...
                    if (target.sizeNum > 3 || target.sizeNum < 3) {
                        const message = (game.i18n.format('CHAT.TestModifiers.ShootingSizeModifier', { size: game.arrant.config.actorSizes[target.details.size.value] }))
    ...
    runEffects(trigger, args, options = {}) {
        let effects = this.actorEffects.filter(e => e.trigger == trigger && (e.script ?? e.flags.arrant.script) && !e.disabled)
        ...
        if (["prepareItem", "prePrepareItem"].includes(trigger)) {
            effects = effects.filter(e => {
                if (e.getFlag("arrant", "promptItem") && e.getFlag("arrant", "itemChoice")) {
                    // If itemChoice is the same as the provided item argument, include it
                    let choiceId = e.getFlag("arrant", "itemChoice")
        ...
        if (trigger == "targetPrefillDialog" && game.user.targets.size) {
            ...
            let secondaryEffects = game.user.targets.values().next().value.actor.actorEffects.filter(e => getProperty(e, "flags.arrant.secondaryEffect.effectTrigger") == "targetPrefillDialog" && !e.disabled) // A kludge that supports 2 effects. Specifically used by conditions
            effects = effects.concat(secondaryEffects.map(e => {
                let newEffect = e.toObject()
                newEffect.flags.arrant.effectTrigger = newEffect.flags.arrant.secondaryEffect.effectTrigger;
                newEffect.flags.arrant.script = newEffect.flags.arrant.secondaryEffect.script;
                return new EffectArrant(newEffect, { parent: e.parent })
            }))
        }

        if (game.arrant.config.syncEffectTriggers.includes(trigger))
        {
            let appliedEffects = [];
            for (let e of effects) {
                ...
                game.arrant.utility.runSingleEffect(e, this, e.item, args, options);
        ...
        {
            return Promise.all(effects.map(e => game.arrant.utility.runSingleEffect(e, this, e.item, args, options)));
        }
    }
    ...
    async decrementInjury(injury) {
        ...
        if (injury.system.duration.value == 0) {
            let chatData = game.arrant.utility.chatDataSetup(game.i18n.format("CHAT.InjuryFinish", { injury: injury.name }), "gmroll")
    ...
    async decrementDisease(disease) {
        ...
        if (!d.system.duration.active) {
            if (Number.isNumeric(d.system.incubation.value)) {
                ...
                let chatData = game.arrant.utility.chatDataSetup(`Attempted to decrement ${d.name} incubation but value is non-numeric`, "gmroll", false)
                chatData.speaker = { alias: this.name }
                ChatMessage.create(chatData)
            }
        }
            ...
            if (Number.isNumeric(d.system.duration.value)) {
                ...
                let chatData = game.arrant.utility.chatDataSetup(`Attempted to decrement ${d.name} duration but value is non-numeric`, "gmroll", false)
                chatData.speaker = { alias: this.name }
                ChatMessage.create(chatData)
            }
        }
    ...
    async activateDisease(disease) {
        ...
        let chatData = game.arrant.utility.chatDataSetup(msg, "gmroll", false)
        chatData.speaker = { alias: this.name }
        ChatMessage.create(chatData)
    }
                ...
                this.setupSkill(game.i18n.localize("NAME.Endurance"), { difficulty }).then(setupData => this.basicTest(setupData).then(async test => {
                    if (test.result.outcome == "failure") {
                        ...
                        if (negSL <= 1) {
                            ...
                            fromUuid("Compendium.arrant-core.diseases.kKccDTGzWzSXCBOb").then(disease => {
                                this.createEmbeddedDocuments("Item", [disease.toObject()])
                            })
                        }
                        else if (negSL >= 6) {
                            msg += game.i18n.localize("CHAT.LingeringRot")
                            fromUuid("Compendium.arrant-core.diseases.M8XyRs9DN12XsFTQ").then(disease => {
                                this.createEmbeddedDocuments("Item", [disease.toObject()])
                            })
                        }
                    }
    ...
    async handleIncomeTest(roll) {
        ...
        let dieAmount = game.arrant.config.earningValues[tier] // b, s, or g maps to 2d10, 1d10, or 1 respectively (takes the first letter)
    ...
    async checkCorruption() {
        ...
        if (this.status.corruption.value > this.status.corruption.max) {
            ...
            if (skill) {
                ...
                this.setupCharacteristic("t", { title: game.i18n.format("DIALOG.MutateTitle", { test: game.arrant.config.characteristics["t"] }), mutate: true }).then(setupData => {
                    this.basicTest(setupData)
                });
            }
        }
    ...
    async handleMutationResult(test) {
        ...
        if (failed) {
            let wpb = this.characteristics.wp.bonus;
            let tableText = game.i18n.localize("CHAT.MutateTable") + "<br>" + game.arrant.config.corruptionTables.map(t => `@Table[${t}]<br>`).join("")
    ...
    async handleExtendedTest(test) {
        ...
        if (game.settings.get("arrant", "extendedTests") && test.result.SL == 0)
        ...
        if (item.system.SL.current >= item.system.SL.target) {
            ...
            if (getProperty(item, "flags.arrant.reloading")) {
                let actor
                if (getProperty(item, "flags.arrant.vehicle"))
                    actor = WFRP_Utility.getSpeaker(getProperty(item, "flags.arrant.vehicle"))

                actor = actor ? actor : this
                let weapon = actor.items.get(getProperty(item, "flags.arrant.reloading"))
                await weapon.update({ "flags.arrant.-=reloading": null, "system.loaded.amt": weapon.loaded.max, "system.loaded.value": true })
            }
    ...
    async checkReloadExtendedTest(weapon) {
        ...
        let reloadingTest = this.items.get(weapon.getFlag("arrant", "reloading"))

        if (weapon.loaded.amt > 0) {
            if (reloadingTest) {
                await reloadingTest.delete()
                await weapon.update({ "flags.arrant.-=reloading": null })
                ui.notifications.notify(game.i18n.localize("ITEM.ReloadFinish"))
                return;
            }
        }
        else {
            let reloadExtendedTest = duplicate(game.arrant.config.systemItems.reload);
            ...
            reloadExtendedTest.flags.arrant.reloading = weapon.id
            ...
            if (weapon.actor.type == "vehicle") {
                ...
                reloadExtendedTest.flags.arrant.vehicle = vehicleSpeaker
            }
            ...
            await weapon.update({ "flags.arrant.reloading": item[0].id });
        }
    }
    ...
    async addCondition(effect, value = 1) {
        if (typeof (effect) === "string")
            effect = duplicate(game.arrant.config.statusEffects.find(e => e.id == effect))
        if (!effect)
        ...
        if (existing && !existing.isNumberedCondition)
            ...
            return existing.setFlag("arrant", "value", existing.conditionValue + value)
        }
        else if (!existing) {
            if (game.combat && (effect.id == "blinded" || effect.id == "deafened"))
                effect.flags.arrant.roundReceived = game.combat.round
            effect.name = game.i18n.localize(effect.name);

            if (Number.isNumeric(effect.flags.arrant.value))
                effect.flags.arrant.value = value;
    ...
    async removeCondition(effect, value = 1) {
        ...
        if (existing && !existing.isNumberedCondition) {
            ...
            await existing.setFlag("arrant", "value", existing.conditionValue - value);
            ...
            if (existing.conditionValue == 0 && (effect.id == "bleeding" || effect.id == "poisoned" || effect.id == "broken" || (effect.id == "stunned" && !this.hasCondition("fatigued")))) {
                if (!game.settings.get("arrant", "mooConditions") || !effect.id == "broken") // Homebrew rule prevents broken from causing fatigue
                    await this.addCondition("fatigued")
            }
    ...
    applyFear(value, name = undefined) {
        value = value || 0
        let fear = duplicate(game.arrant.config.systemItems.fear)
        fear.system.SL.target = value;

        if (name)
            fear.effects[0].flags.arrant.fearName = name
    ...
    applyTerror(value, name = undefined) {
        value = value || 1
        let terror = duplicate(game.arrant.config.systemItems.terror)
        terror.flags.arrant.terrorValue = value
        return game.arrant.utility.applyOneTimeEffect(terror, this)
    }
    ...
    populateEffect(effectId, item, test) {
        ...
        let script = getProperty(effect, "flags.arrant.script")
        if (test && script) {
            let regex = /{{(.+?)}}/g
            ...
            setProperty(effect, "flags.arrant.script", script)
        }
    ...
    async addSystemEffect(key) {
        let systemEffects = game.arrant.utility.getSystemEffects()
    ...
    displayStatus(round = undefined, nameOverride) {
        ...
        chatOptions["template"] = "systems/arrant/templates/chat/combat-status.hbs"
    ...
    {
        let conditionalEffects = foundry.utils.deepClone(this.flags.arrant?.conditionalEffects || [])
        ...
        setProperty(this, "flags.arrant.conditionalEffects", conditionalEffects);
    }
    ...
    // @@@@@@@@@@@ COMPUTED GETTERS @@@@@@@@@
    get Species() {
        let species = game.arrant.config.species[this.details.species.value] || this.details.species.value
        if (this.details.species.subspecies && game.arrant.config.subspecies[this.details.species.value] && game.arrant.config.subspecies[this.details.species.value][this.details.species.subspecies])
            species += ` (${game.arrant.config.subspecies[this.details.species.value][this.details.species.subspecies].name})`
    ...
    get sizeNum() {
        return game.arrant.config.actorSizeNums[this.details.size.value]
    }
        ...
        return this.system.passengers.map(p => {
            ...
            if (actor)
                return {
                    ...
                    enc: game.arrant.config.actorSizeEncumbrance[actor.details.size.value] * p.count
                }
        })
    }
