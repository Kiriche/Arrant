...
export default class WFRPActiveEffectConfig extends ActiveEffectConfig {
    ...
    async getData() {
        ...
        data.effectTriggers = game.arrant.config.effectTriggers;
        let type = getProperty(data, "effect.flags.arrant.effectTrigger")
        if (type && type != "dialogChoice")
        {
            data.showEditor = true;
            data.placeholder = game.arrant.config.effectPlaceholder[type] + game.arrant.config.effectPlaceholder.this
            if (game.arrant.config.syncEffectTriggers.indexOf(type) === -1)
        ...
        if (this.object.flags.arrant?.promptItem)
        {
            data.showExtra = true;
            data.extraPlaceholder = game.i18n.localize("EFFECT.ItemFilters");
        }

        data.effectApplication = duplicate(game.arrant.config.effectApplication)
        if (this.object.parent.documentName == "Item")
        {
            ...
            if (this.object.parent.type == "spell" || this.object.parent.type == "prayer")
            {
                ...
                if (this.object.parent.system.target.value == game.i18n.localize("You") && this.object.parent.system.range.value == game.i18n.localize("You"))
                    data.notSelfOption = game.i18n.localize("arrant.NotSelf");
            }
        ...
        {
            data.effect.flags.arrant.effectTrigger = "applyDamage"
    ...
    get template() {
        return "systems/arrant/templates/apps/active-effect-config.hbs"
    }

    async _updateObject(event, formData) {
        if (this.aceEditor)
        {
            formData["flags.arrant.script"] = this.aceEditor.getValue();
        }
    ...
    activateListeners(html){
        ...
        try 
        {
            if (game.modules.get("acelib")?.active)
            {
                ...
                this.aceEditor.setValue(this.object.flags.arrant?.script);
            }
        }
        catch(e)
        {
